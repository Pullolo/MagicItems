...
public class FireScroll extends Scroll {
    ...
    public FireScroll() {
        ...
        // Load damage from config.yml
        FileConfiguration config = Bukkit.getPluginManager().getPlugin("MagicItems").getConfig();
        damage = config.getDouble("fire_scroll.damage", 4.0); // default to 4.0 if missing
    }
    ...
    public void executeAbility(Player p) {
        ...
        for (Entity e : p.getLocation().getNearbyEntities(4, 2, 4)) {
            ...
            e.setFireTicks(65);
            ((LivingEntity) e).damage(damage, p); // use configurable damage
        }
